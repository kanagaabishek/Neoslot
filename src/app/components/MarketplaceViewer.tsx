        ...
        {sales.map((sale) => (
          <div key={sale.sale_id} className="border border-gray-200 rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="font-medium text-gray-900">Sale ID: {sale.sale_id}</span>
              <span className={`px-2 py-1 rounded text-xs ${
                sale.status === 'open' ? 'bg-green-100 text-green-800' : 
                sale.status === 'executed' ? 'bg-gray-100 text-gray-800' :
